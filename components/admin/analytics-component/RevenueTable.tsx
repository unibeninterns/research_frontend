...
const RevenueTable = ({
  ...
  const tableInfo = info.labels.map(function (label, i) {
    return {
      course: label,
      earned: info.earned[i],
      payout: info.payout[i],
      profitLoss: info.earned[i] - info.payout[i],
      profitMargin: (info.earned[i] - info.payout[i]) / info.earned[i],
    };
  });
  return (
    <div>
            ...
            <tbody className="divide-y divide-gray-200 bg-white">
              {tableInfo.map((info) => (
                ...
                >
                  ...
                  <td className="p-3 text-sm">₦{info.earned}</td>
                  <td className="p-3 text-sm">₦{info.payout}</td>
                  ...
                  <td
                    className={`p-3 text-sm ${info.profitMargin > 0 ? "text-[#1DAE00]" : info.profitMargin < 0 ? "text-[#FF383C]" : ""}`}
                  >
                    {`${(info.profitMargin * 100).toFixed(2)}%`}
                  </td>
                </tr>
              ))}
            </tbody>
