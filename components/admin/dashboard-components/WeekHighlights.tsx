...
function WeekHighlights() {
  return (
    <div className='bg-white px-[24px] py-[16px] border-[0.5px] border-[#D9D9D9] flex flex-col rounded-[5px]'>
      <div>
        <h2 className='text-[16px] font-semibold mb-[20px]'>{`This Week's Highlights`}</h2>
      </div>
        ...
        {highlights.map((highlight, index) => (
          <div key={index} className='flex items-start gap-3'>
            ...
            <div className='flex-1'>
              ...
              {highlight.time && (
                <p className='text-[10px] text-gray-500 mt-1'>{highlight.time}</p>
              )}
            </div>
          </div>
        ))}
