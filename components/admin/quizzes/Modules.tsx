    ...
    <div className="mt-6 flex flex-col gap-4">
      ...
      <div className="grid-col grid grid-cols-2 gap-x-8 gap-y-6">
        {courseQuiz.quizzes.map((quiz) => (
          <Module key={quiz.topic} quiz={quiz} />
        ))}
      </div>
    </div>
  );
};
const Module = ({ quiz }: { quiz: QuizModule }) => {
  ...
  return (
    <div className="flex flex-col gap-5 rounded-[5px] border px-4 py-6">
      <div className="flex justify-between">
        <h3 className="text-base font-semibold">{`Module ${quiz.module}: ${quiz.topic}`}</h3>
      ...
      <div className="flex items-start gap-8 text-xs">
        <div className="flex flex-1 flex-col gap-2">
          <div className="flex justify-between">
            <p>Quiz Questions</p>
            <p>{quiz.quizQuestions}</p>
          </div>
          <div className="flex justify-between">
            <p>Time Limit</p>
            <p>{quiz.timeLimit ? quiz.timeLimit : "NIL"}</p>
          </div>
        </div>
        <div className="flex flex-1 flex-col gap-2">
          <div className="flex justify-between">
          ...
          <div className="flex justify-between">
            <p>Grading Type</p>
            <p>{quiz.gradingType}</p>
          </div>
        </div>
        <p
          className={`rounded-[5px] px-2 py-1 text-[10px] ${getBadge(quiz.mode)} `}
        >
          {quiz.mode}
        </p>
      </div>
