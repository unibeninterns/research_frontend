...
function StatCard({
  ...
  return (
    <div className="cursor-pointer rounded-lg border border-gray-100 bg-white p-4 shadow-sm transition-shadow hover:shadow-md lg:p-6">
      <div className="flex items-center gap-3">
        <div className={`rounded-full p-2 lg:p-3 ${color} flex-shrink-0`}>
          {icon}
        </div>
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm text-gray-600 lg:text-base">{title}</p>
...
export function StudentStats(): React.JSX.Element {
  ...
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4 lg:grid-cols-5 lg:gap-6">
      {stats.map((stat) => (
